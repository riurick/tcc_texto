...
\begin{enumerate}
	\item Utilidade: é a medida na qual o sistema propicia o tipo certo de funcionalidade, de maneira que os usuários possam realizar aquilo de que precisam ou desejam.
	\item Eficácia: refere-se ao quanto um sistema é bom em fazer o que se espera dele.
	\item Eficiência: refere-se à maneira como o sistema auxilia os usuários na realização de suas tarefas.
	\item Segurança: implica em proteger o usuário de condições perigosas e sitações indesejáveis.
	\item Facilidade de aprendizado: refere-se a qual fácil é aprender a usar o sistema.
	\item Facilidade de lembrar como se usa: refere-se à facilidade de lembrar como utiliza um sistema, depois de já ter aprendido como usar.
\end{enumerate}

% escrever mais detalhes ou inserir em outro lugar.
%TODO: precisa explicar brevemente cada item

%\section{As áreas da Usabilidade}
%TODO: Não precisa ser uma seção e nem de subseções aqui
...
%
As pessoas que utilizarão um produto ou serviço sabem de suas necessidades, metas e preferências, e é papel do \emph{design} descobrir isso para projetar para eles -- e essa é a filosofia por trás do \emph{design} centrado no usuário~\cite{saffer2010designing}.
...
%
Já ~\citeonline{cybis2010} divide as técnicas de avaliação da usabilidade de interfaces em três grupos. Técnicas prospectivas; técnicas preditivas, diagnósticas, analíticas e inspecção e as objetivas, definitivas e empíricas.
...
%
O principal objetivo do teste de aceitação do usuário é servir como uma verificação final de que a aplicação atendeu aos requisitos funcionais estabelecidos pelo cliente ~\citeonline{preece2007}.
...
~\citeonline{lee2011} relata em seus estudos a utilização da abordagem de usabilidade ágil eXtreme Scenario-based \emph{design} (XSBD) na qual concluiram que os desafios que o time encontrava eram relacionados à comunicação, colaboração e compartilhamento de informação.
...
\subsection{DCU Ágil}
	...
	Segundo \citeonline{najafi2008} o \emph{design} centrado no usuário pode ser incorporado no desenvolvimento ágil sem grandes impactos no processo. No método de desenvolvimento ágil scrum, uma funcionalidade é desenvolvida, testada e documentada em uma sprint assim como no \emph{design} centrado no usuário. 
...
\subsection {DCU e XP}
	...
	Foram encontrados alguns estudos que descrevem a integração do IHC com as práticas de Programação Extrema (XP). Segundo ~\citeonline{mcinerney2005ucd} métodos ágeis e \emph{design} centrado no usuário possuem uma cultura distinta mas eles mostraram que é possivel unir essas duas abordagens.
...
\end{figure}
	...
	O papel de \emph{design}er de interação não é reconhecido no núcleo eXtreme Programming (XP) da equipe e o XP não tem nenhum processo explícito para lidar com o \emph{design} de interação.Em seu estudo ~\citeonline{ferreira2007interaction} relatam como as equipes combinaram as atividades de \emph{design} de interação com o XP. 

	A natureza iterativa de desenvolvimento XP necessário que os \emph{design}ers de interação tem envolvimento contínuo com o desenvolvimento do produto influenciou a natureza da relação entre os \emph{design}ers de interação e os desenvolvedores.
...
\item Criar \emph{design}s alternativos
...
\item Prototipar em baixa resolução
	...
	Os protótipos de baixa resolução permite visualizar uma solução de forma mais rápida e concreta economizando tempo de \emph{design} e desenvolvimento. Esses prótotipos ajuda a comunicar e entender idéias e serve para validar uma solução. Eles são úteis pois tendem a ser simples, baratos e de rápida produção. 
%
	A prototipação aumenta a comunicação entre a equipe de desenvolvimento e os usuários finais, funcionando como uma alternativa ``barata" para explorar alternativas de desenho. Um exemplo de prótotipo de baixa fidelidade seria os storyboards que foi detalhado na seção de técnicas de usabilidade.
	
\item Menos documentação, mais comunicação

	É essencial que tenha uma boa comunicação em todo o processo entre os desenvolvedores, \emph{design}s e stakeholders envolvidos no projeto.
